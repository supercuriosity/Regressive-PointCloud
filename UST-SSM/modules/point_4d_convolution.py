...
class P4DTransConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, original_xyzs: torch.Tensor, features: torch.Tensor, original_features: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        r"""
        Parameters
        ----------
        xyzs : torch.Tensor
        ...
        # keep the original full-tensor to return as new_xyzs
        new_xyzs = original_xyzs

        # Do not reassign the parameter-typed tensors; use local list variables instead
        xyzs_list = torch.split(tensor=xyzs, split_size_or_sections=1, dim=1)
        xyzs_list = [torch.squeeze(input=xyz, dim=1).contiguous() for xyz in xyzs_list]

        features_list = torch.split(tensor=features, split_size_or_sections=1, dim=1)
        features_list = [torch.squeeze(input=feature, dim=1).contiguous() for feature in features_list]

        original_xyzs_list = torch.split(tensor=original_xyzs, split_size_or_sections=1, dim=1)
        original_xyzs_list = [torch.squeeze(input=original_xyz, dim=1).contiguous() for original_xyz in original_xyzs_list]

        if original_features is not None:
            original_features_list = torch.split(tensor=original_features, split_size_or_sections=1, dim=1)
            original_features_list = [torch.squeeze(input=feature, dim=1).contiguous() for feature in original_features_list]
        else:
        ...
        for t in range(T):
            dist, idx = pointnet2_utils.three_nn(original_xyzs_list[t], xyzs_list[t])

            dist_recip = 1.0 / (dist + 1e-8)
            norm = torch.sum(dist_recip, dim=2, keepdim=True)
            weight = dist_recip / norm

            interpolated_feat = pointnet2_utils.three_interpolate(features_list[t], idx, weight)

            if original_features_list is not None:
                new_feature = torch.cat([interpolated_feat, original_features_list[t]], dim=1)
            else:
