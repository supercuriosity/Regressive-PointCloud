...
def gray2binary(gray, axis=-1) :
    # Compute initial shift as the highest power of two strictly less than the axis length.
    n = gray.shape[axis]
    if n <= 1:
        return gray
    shift = 1 << ((n - 1).bit_length() - 1)
    while shift > 0:
        gray = torch.logical_xor(gray, right_shift(gray, int(shift), axis=axis))
        shift //= 2
    return gray
