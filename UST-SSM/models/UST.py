...
# Prefer package-relative imports so editors can resolve symbols when the
# project is opened as a workspace. Keep sys.path fallbacks for scripts.
try:
    # When imported as a package (UST-SSM.models.UST)
    from ..modules.point_4d_convolution import P4DConv, P4DTransConv, AggregationSSM, GroupFeature, STSAL
except :
    try:
        # When modules dir is on sys.path (scripts executed from repo root)
        from point_4d_convolution import P4DConv, P4DTransConv, AggregationSSM, GroupFeature, STSAL
    except :
        # Last-resort fallback: try import via explicit modules package name
        try:
            from modules.point_4d_convolution import P4DConv, P4DTransConv, AggregationSSM, GroupFeature, STSAL
        except :
            # If none of the above worked, raise ImportError so failures are
            # visible at runtime (better than silently using Any).
            raise
...
try:
    import importlib
    layernorm_mod = importlib.import_module("mamba_ssm.ops.triton.layernorm")
    RMSNorm = getattr(layernorm_mod, "RMSNorm", None)
    layer_norm_fn = getattr(layernorm_mod, "layer_norm_fn", None)
    rms_norm_fn = getattr(layernorm_mod, "rms_norm_fn", None)
except ImportError:
...
def create_block(
    ...
    # Choose a concrete base class for normalization: prefer RMSNorm when requested and available,
    # otherwise fall back to nn.LayerNorm. This avoids passing None into a partial/class factory.
    if rms_norm and (RMSNorm is not None):
        _norm_base = RMSNorm
    else:
        _norm_base = nn.LayerNorm

    # Create a dynamic subclass of the chosen norm base so we can bake eps/device/dtype
    # into a class object (type), which matches the expected type[LayerNorm] annotation.
    def _make_norm_cls(base_cls, eps, factory_kwargs):
        class NormCls(base_cls):
            def __init__(self, normalized_shape, *args, **kwargs):
                # Pass normalized_shape through and inject eps/device/dtype from factory_kwargs.
                # Using super().__init__ keeps compatibility with nn.LayerNorm and similar APIs.
                super().__init__(normalized_shape, eps=eps, **factory_kwargs)
        return NormCls

    norm_cls = _make_norm_cls(_norm_base, norm_epsilon, factory_kwargs)
    ...
    # Ensure layer_idx is an integer (avoid assigning None) for downstream code and type checkers.
    # Use object.__setattr__ to avoid nn.Module.__setattr__ type restrictions that disallow plain ints.
...
class UST(nn.Module):
    ...
    def forward(self, input):
        ...
        outputs = []
        # fallback: if no branch sets main_x_output (e.g., empty branches), use current features as default
        # features currently has shape [B, L, C, n], so permute to [B, L, n, C] to match expected feat shape
        main_x_output = features.permute(0, 1, 3, 2).reshape(B, L, n, -1)  
