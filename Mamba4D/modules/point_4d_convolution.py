...
class P4DConv(nn.Module):
    def __init__(self,
        ...
        self.temporal_kernel_size = temporal_kernel_size # 3
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        new_xyzs = []
        new_features = []
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames, 1, 23, 2
...
class P4DTransConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, original_xyzs: torch.Tensor, features: torch.Tensor, original_features: torch.Tensor = None) -> torch.Tensor:
        ...
        for t in range(T):
            #dist 是形状为 (B, N, 3) 的张量，表示每个点在 xyz2 中的三个最近邻点的距离。
            #idx 是形状为 (B, N, 3) 的张量，表示每个点在 xyz2 中的三个最近邻点的索引。
