...
class IntraMamba(nn.Module):
    def __init__(self,
        ...
        self.smallMamba = MixerModel(d_model=mlp_planes[0],
                            n_layer=depth_mamba_intra,
                            rms_norm=False,
                            drop_out_in_block=0.1,
                            drop_path=0.1)

        self.group_divider = Group(num_group= 2048/self.spatial_stride, group_size=self.k)

        self.encoder = Encoder(encoder_channel=mlp_planes[0])

        self.pos_embed = nn.Sequential(
            nn.Linear(4, 128),
            nn.GELU(),
            nn.Linear(128, mlp_planes[0])
        )

        self.norm = nn.LayerNorm(mlp_planes[0])
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            for i in range(t-self.temporal_kernel_size//2, t+self.temporal_kernel_size//2+1):
                ...
                neighborhood = neighbor_xyz_grouped.permute(0, 2, 3, 1).contiguous()
                tokens = self.encoder(neighborhood)                                               
                t_displacement = torch.ones((anchor_xyz.size()[0], anchor_xyz.size()[1], 1), dtype=torch.float32, device=device) * (i-t)
                center = torch.cat(tensors=(anchor_xyz, t_displacement), dim=2, out=None)                               
                pos = self.pos_embed(center)

                feature = self.smallMamba(tokens, pos)
