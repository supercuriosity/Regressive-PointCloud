...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, print_freq):
    """
    训练单个epoch的核心函数：遍历训练集，完成前向传播、损失计算、反向传播和参数更新
    """
    model.train()
...
def evaluate(model, criterion, data_loader, device, print_freq):
    ...
    logger.info(' * Eval accuracy: %f'% (np.mean(np.array(ACCs))))
    ...
    logger.info(' * Eval mIoU:\t %f'%(np.mean(np.array(IoUs))))
...
def main(args):
    ...
    if args.output_dir:
        ...
        os.system('cp %s %s' % ('./modules-pytorch-1.8.1/point_4d_convolution.py', args.output_dir))
        os.system('cp %s %s' % ('./models/synthia.py', args.output_dir))
        # 复制关键代码文件到输出目录（便于复现实验：保存训练时使用的模型和卷积模块代码）
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        evaluate_acc = evaluate(model, criterion_test, data_loader_test, device=device, print_freq=args.print_freq)
    ...
    logger.info('mIoU {}'.format(acc))

def parse_args():
    parser = argparse.ArgumentParser(description='Transformer Model Training')

    parser.add_argument('--data-path', default='/dataset/SYNTHIA/sequence', help='data path')
    parser.add_argument('--data-train', default='/dataset/SYNTHIA/trainval_raw.txt', help='meta list for training')
    parser.add_argument('--data-eval', default='/dataset/SYNTHIA/test_raw.txt', help='meta list for test')
    parser.add_argument('--label-weight', default='/dataset/SYNTHIA/label/labelweights.npz', help='training label weights')
    ...
    parser.add_argument('--output-dir', default='/data/MAMBA4D/output/synthia', type=str, help='path where to save')
    # resume
